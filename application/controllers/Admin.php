...
class Admin extends CI_Controller {
    ...
    function invoice_caja($id = ''){    #guardar caja
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        
        $data['student_id'] = $this->input->post('student_id');
        $data['title'] = $this->input->post('txt_concept');
        $data['description'] = $this->input->post('txt_concept');
        $data['amount'] = $this->input->post('txt_total');
        $data['amount_paid'] = $this->input->post('txt_total');
        $data['due'] = $data['amount'] - $data['amount_paid'];  #Saldo
        $data['status'] = ($data['due'] <= 0 ? "Paid" : "UnPaid");
        $data['creation_timestamp'] = strtotime(date("Y-m-d H:i:s"));   #strtotime($this->input->post('date'));
        $data['year'] = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;

        $this->db->insert('invoice', $data);    #caja
        $invoice_id = $this->db->insert_id();

        $data2['invoice_id']    = $invoice_id;
        $data2['student_id']    = $this->input->post('student_id');
        $data2['title']         = $this->input->post('txt_concept');
        $data2['description']   = $this->input->post('txt_concept');
        $data2['payment_type']  = 'income';
        $data2['method']        = 1;
        $data2['amount'] = $this->input->post('txt_total');    # $this->input->post('amount_paid'); monto pagado
        ...
        #insertar semilla 
        $data5['id_centro_educativo']   = $this->session->userdata('VS_idCentroEducativo'); #variable Session
        $data5['id_tipo_docu_caja']     = $this->input->post('id_tipo_docu_caja');        
        $data5['nu_anio']               = date("Y");
        $data5['nu_serie']              = 1;
        $idTipoDocuCaja                 = (int)$this->input->post('id_tipo_docu_caja'); 
        $data5['nu_documento']          = $this->crud_model->get_numero_comprobante($this->session->userdata('VS_idCentroEducativo'), $idTipoDocuCaja);
        $data5['id_usua_crea']          = $this->session->userdata('admin_login');
        $data5['ts_usua_crea']          = date("Y-m-d H:i:s");
        $this->db->insert('a_semi_tdoc_caja', $data5);
        $id_semi_tdoc_caja = $this->db->insert_id();
         
        #Caja Cabecera     ======================================
        $data3['id_centro_educativo']   = $this->session->userdata('VS_idCentroEducativo'); #variable Session
        $data3['student_id']            = $this->input->post('student_id');
        $data3['invoice_id']            = $invoice_id;
        $data3['id_tipo_docu_caja']     = $this->input->post('id_tipo_docu_caja');
        $data3['id_semi_tdoc_caja']     = $id_semi_tdoc_caja;
        $data3['nu_caja']               = $data5['nu_documento'];
        $data3['fe_caja']               = date("Y-m-d");
        $data3['id_tipo_forma_pago']    = 1;    #contado
        $data3['mo_bruto']              = $this->input->post('txt_pension');
        $data3['mo_igv']                = 0;
        $data3['mo_mora']               = is_null($this->input->post('txt_mora')) ? 0 :$this->input->post('txt_mora');
        $data3['mo_neto']               = $this->input->post('txt_total');
        $data3['id_esta_caja']          = 1;
        $this->db->insert('p_caja', $data3);
        $id_caja = $this->db->insert_id();
        
        #Detalle de Caja    ======================================
        $data4['id_caja']           = $id_caja;
        $data4['id_compromiso_pago']= $this->input->post('id_compromiso_pago');
        $data4['id_pago_concepto']  = $this->input->post('id_concep_pago');
        $data4['de_pago_concepto']  = $this->input->post('txt_concept') . " " .$this->input->post('hdn_nu_anio')."-".$this->input->post('hdn_cuota');
        $data4['nu_cantidad']       = 1;
        $data4['nu_cuota']          = $this->input->post('hdn_cuota');
        $data4['nu_precio']         = $this->input->post('txt_amount'); 
        $data4['mo_bruto']          = $this->input->post('txt_pension');
        $data4['mo_igv']            = 0;
        $data4['mo_mora']           = is_null($this->input->post('txt_mora') ? 0 : $this->input->post('txt_mora'));
        $data4['mo_neto']           = $data4['mo_bruto']  + $data4['nu_precio'] ;         
        $this->db->insert('h_deta_caja', $data4);
        
        #Actualizar Comprmiso de Pago
        $datau['id_caja'] = $id_caja;
        $datau['fe_caja'] = $data3['fe_caja'];

        $this->db->where('id_compromiso_pago', $id);
        $this->db->update('r_compromisos_pago', $datau);
            
        #redirect(base_url() . 'index.php?admin/payment_commitment', 'refresh');
        #$data_user['data'] = $this->db->get('student')->result();
        
        $data_user['nu_comprobante'] = $data3['nu_caja'];
        $data_user['student_id']	 = $data3['student_id'];
        $data_user['fe_caja']   	 = $data3['fe_caja'];                
        #$data_user['deta_caja'] = $this->db->get('h_deta_caja',array('id_caja'=>6))->result();
			$this->db->select('*'); 
			$this->db->where('id_caja',$id_caja);
		#$query = $this->db->get('h_deta_caja'); 
		$data_user['deta_caja']  = $this->db->get('h_deta_caja')->result();
        
        $this->load->view('backend/admin/voucher', $data_user);
        #get output html
        ...
        #setup paper size and orientation array(0,0,360,360)
    ...
    function marks_area($student_id = '') {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        $class_id = $this->db->get_where('enroll', array(
                    'student_id' => $student_id, 'year' => $this->db->get_where('settings', array('type' => 'running_year'))->row()->description))->row()->class_id;
        $student_name = $this->db->get_where('student', array('student_id' => $student_id))->row()->name;
        $class_name = $this->db->get_where('class', array('class_id' => $class_id))->row()->name;
    ...
    function student($param1 = '', $param2 = '', $param3 = '') { #Estudiantes
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;

        if ($param1 == 'create') {  #Inserta un nuevo Estudiante
            $data['name'] = $this->input->post('name');
            $data['username'] = $this->input->post('username');
            $data['birthday'] = $this->input->post('birthday');
            $data['date'] = strtotime(date("d M,Y"));
            ...
            $this->db->insert('student', $data);
            $student_id = $this->db->insert_id(); #recupero el id
            ...
            $ds_fecha_venci = $this->db->get_where('a_fecha_venci', array('id_centro_educativo' => $id_centro_educativo))->result_array();
            foreach ($ds_fecha_venci as $row) {
                if ($row['nu_cuota'] == 1) {
                    $pago_data1['id_centro_educativo'] = $id_centro_educativo;
                    $pago_data1['student_id'] = $student_id;
                    $pago_data1['id_pago_concepto'] = 1;    #Pago de Pension
                    $pago_data1['fe_vencimiento'] = $row['fe_vencimiento'];
                    $pago_data1['nu_cuota'] = $row['nu_cuota'];
                    $pago_data1['mo_cuota_pagar'] = 100;
                    $pago_data1['mo_neto'] = 0;
                    $pago_data1['mo_deuda_pago'] = 0;
                    $pago_data1['id_usua_crea'] = $this->session->userdata('admin_login');
                    $pago_data1['ts_usua_crea'] = date("Y-m-d H:i:s");
                    #solo en la primera cuota agrega la matricula
                    $this->db->insert('r_compromisos_pago', $pago_data1);
                }
                $pago_data['id_centro_educativo'] = $id_centro_educativo;
                $pago_data['student_id'] = $student_id;
                $pago_data['id_pago_concepto'] = 2;    #Pago de Pension
                $pago_data['fe_vencimiento'] = $row['fe_vencimiento'];
                $pago_data['nu_cuota'] = $row['nu_cuota'];
                $pago_data['mo_cuota_pagar'] = $this->input->post('mo_cuota_pagar');
                $pago_data['mo_neto'] = 0;
                $pago_data['mo_deuda_pago'] = 0;
                $pago_data['id_usua_crea'] = $this->session->userdata('admin_login');
                $pago_data['ts_usua_crea'] = date("Y-m-d H:i:s");
                #inserta compromisos de pago
                $this->db->insert('r_compromisos_pago', $pago_data);
            }
            redirect(base_url() . 'index.php?admin/add_student/', 'refresh');
        }

        if ($param1 == 'do_update') {    #Actualiza un nuevo Estudiante
            $data['name'] = $this->input->post('name');
            $data['username'] = $this->input->post('username');
            $data['phone'] = $this->input->post('phone');
            $data['address'] = $this->input->post('address');
    ...
    function payment_commitment($param1 = '', $param2 = '') {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        $page_data['page_name'] = 'payment_commitment';
        $page_data['page_title'] = get_phrase('payment-commitment');
        $this->db->order_by('creation_timestamp', 'desc');
        $page_data['invoices'] = $this->db->get('invoice')->result_array();
        $this->load->view('backend/index', $page_data);
    }
