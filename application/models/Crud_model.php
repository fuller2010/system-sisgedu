...
class Crud_model extends CI_Model 
{
    ...
    #Funciones FECHA ###################################################################################
    function expiration_day($fec_vencimiento){
        if(date('Y-m-d')>=$fec_vencimiento ){
            list($anio, $mes, $dia) = explode("-", $fec_vencimiento);
            $timestamp_venci = mktime(0, 0, 0, $mes, $dia, $anio); #convierte a segundos
            $timestamp_hoy   = mktime(0, 0, 0, date("m"), date("d"), date("Y"));

            //resto a una fecha la otra 
            $segundos_diferencia = ($timestamp_hoy - $timestamp_venci); 
            $segundos_diferencia -=86400; #le resto un dia
            $dias_vencidos = $segundos_diferencia / (60 * 60 * 24);
    ...
    #fecha peru
    Function see_date_peru($date){
        list($anio, $mes, $dia) = explode("-", $date);
        $fecha = $dia."/".$mes."/".$anio;
        return $fecha;
    }
    
    #Calculo de la Mora de pensiones
    function default_rate($dias_vencidos){
        $tasa = 0.1;
        return $dias_vencidos*$tasa;
    }
    
    #numero de comprobante
    function get_numero_comprobante($id_centro_educativo, $id_tipo_docu_caja){                
        $num = $this->db->limit(1)->order_by('id_semi_tdoc_caja','DESC')->get_where('a_semi_tdoc_caja', array('in_esta' => 1, 'id_centro_educativo'=>$id_centro_educativo,
                                                              'id_tipo_docu_caja'>=$id_tipo_docu_caja))->row()->nu_documento; 
        return $num+1;
    }
